import { useQuery } from "@tanstack/react-query";
import { getPendingPaymentShipmentDetails } from "../../../services/shipper/shipments.service";
import { toast } from "react-hot-toast";

export function useShipperPendingPaymentDetails(shipmentId) {
  const { data: shipment = null, isLoading, error } = useQuery({
    queryKey: ["pendingPaymentDetails", shipmentId],
    queryFn: async () => {
      const res = await getPendingPaymentShipmentDetails(shipmentId);
      return res?.data?.shipment || null;
    },
    enabled: !!shipmentId,
    onError: (error) => {
      const msg = error?.response?.data?.message || "Failed to fetch shipment details";
      toast.error(msg);
    },
  });

  return {
    shipment,
    isLoading,
    error,
  };
}
